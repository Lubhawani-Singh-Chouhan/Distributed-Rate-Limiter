...
## Headline run — **1,000 req/s passed** (2026-08-18)
...
Open **Requests / sec**: after the 20s ramp the line sits at **~1,000** for a minute. That is
the pass. 70,010 / 70,010 completed, p95 10 ms, 0 KO.

Gatling also prints Cnt/s **864.32** in the Stats table. That is the **full-run average
including the ramp**, not a failed 1k test.

| | |
|---|---|
| Date | 2026-08-18 17:26:54 GMT (duration 1m 20s) |
| Simulation | [`SustainedThroughputSimulation.java`](../src/test/java/simulations/SustainedThroughputSimulation.java) (`TENANT_POOL_SIZE=200`, `target.rps` default 1000) |
...
| Workload | Open model, `GET /api/v1/resource`, 200 rotating `loadtest-tenant-*` keys |
| Offered | 20s ramp 1→1000 rps, then 60s at 1000 rps |
| Completed | **70,010 / 70,010** |
| Errors | **0** (0% KO) |
| Sustain | **~1,000 req/s** (Requests/sec chart) |
| Full-run Cnt/s | 864.32 (ramp included — expected) |
| Latency | mean **5 ms**, p50 5, p75 6, p95 **10 ms**, p99 **19 ms**, max 78 ms |
...
key (burst 50). 200 keys are how 1k limiter **decisions**/sec stay inside the buckets.
...
| Folder | What it is |
|---|---|
| [`gatling/sustainedthroughputsimulation-20260818172653414/`](gatling/sustainedthroughputsimulation-20260818172653414/index.html) | **1k rps passed** — 70,010/70,010, Requests/sec ~1000, Cnt/s 864.32 (ramp included) |
